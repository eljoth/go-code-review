...
// panic due to repository entries not being initialized on memdb.go
...
// Test to check if you can create a duplicate coupon
func TestService_CreateCouponDuplicate(t *testing.T) {
	type fields struct {
		repo Repository
	}
	type args struct {
	...
	tests := []struct {
		...
		{"Apply 10%", fields{memdb.New()}, args{10, "Superdiscount", 55}, nil},
		{"Apply 10% duplicate", fields{memdb.New()}, args{10, "Superdiscount", 55}, nil},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := Service{
